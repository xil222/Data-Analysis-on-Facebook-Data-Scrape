...
def network(label):
    ...
    G = school_dict[label]
    degree_dict = nx.get_node_attributes(G, 'degree')
    #Notes : fruchterman_reingold_layout(G) - layout based on larger hubs near center (force graph)
...
def network2(label):
    ...
    G = school_dict[label]
    degree_dict = nx.get_node_attributes(G, 'degree')
    nx.draw(G, with_labels=False, node_color='darkviolet', node_size=[size*10 for size in degree_dict.values()], edge_color='red', alpha = 0.5, linewidths=2, font_size=5,pos=nx.spring_layout(G))
...
pos=nx.random_layout(G) - Kinda doesnt tell too much, just that more connections means more lines crossing each other (at random)... X
pos=nx.spectral_layout(G) - i dont even know what it's meant to show... X
